export default function auth() {
  ...
  const infoTypesAuths = document.querySelectorAll('.info-types--auth');
  if (infoTypesAuths.length === 0) return;

  infoTypesAuths.forEach((infoTypesAuth) => {
    const form = infoTypesAuth.closest('form');
    const panelTmp = form.querySelector('.panel__tmp');
    const panelInputs = form.querySelector('.inputs');
    const steps = form.querySelector('.web-create__step-panels');
    const activeInfoType = form.querySelector('.info-types__type--is-active');
